...
public sealed record ReservedRange(IPv4Network Network, ReservationKind Kind, string Description, ExistingVnet? Vnet = null)
{
    ...
    private static string DescribeVnet(ExistingVnet vnet)
    {
        ...
        var name = string.IsNullOrWhiteSpace(vnet.VnetName) ? "(unknown VNET)" : vnet.VnetName;
...
public static class AzureReservedRanges
{
    ...
    [
        new(IPv4Network.Parse("127.0.0.0/8"), ReservationKind.AzureReserved, "Loopback (not allowed in Azure)"),
        new(IPv4Network.Parse("169.254.0.0/16"), ReservationKind.AzureReserved, "Link-local (not allowed in Azure)"),
        new(IPv4Network.Parse("168.63.129.16/32"), ReservationKind.AzureReserved, "Azure internal DNS / health probe"),
        new(IPv4Network.Parse("224.0.0.0/4"), ReservationKind.AzureReserved, "Multicast (not allowed in Azure)"),
        new(IPv4Network.Parse("255.255.255.255/32"), ReservationKind.AzureReserved, "Broadcast (not allowed in Azure)"),
    ];
}
